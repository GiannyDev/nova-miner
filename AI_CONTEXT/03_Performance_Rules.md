...
* El chunk **no** decide el contenido: solo acota WHERE. `OreSpawner` genera cada celda **una vez** al revelar (`UNKNOWN` → dirt / ore / hueco). Pipeline: spawn holes → rutas caminables → **vetas Perlin** → extras capados → resto tierra → **bombas** → huecos de perk. `STARTING_ORE_AMOUNT` suma densidad de veta (12 = baseline). Extras al destruir se aplican al *siguiente* lote revelado (tope 20%), nunca rellenan el túnel.
* Ayudantes: `CharacterBody2D` como el player (mask World, drill Area2D). `ensure_generated` (lote Chebyshev local, sin visuales fuera de ventana). Live = mismo `DrillWeapon`; culled = `mine_cell` solo en la celda del tip/body. Cull escribe HP residual. Cero `get_nodes_in_group`. Markers: O(helpers + perks).
...
* Los `Ore` se reciclan por `OrePool` con `on_spawned` / `on_despawned` (nodos huerfanos fuera del arbol). Nunca `queue_free` en un ore. Tierra, mineral y **bomba** son la misma escena; identidad en `OreData`.
* Blast de bomba: Chebyshev + `mine_cell` (vivos y culled). Ripple por anillo.
