...
* El chunk **no** decide el contenido: solo acota WHERE. `OreSpawner` genera cada celda **una vez** al revelar (`UNKNOWN` → dirt / ore / hueco). Pipeline: spawn holes → rutas caminables → clusters → extras capados → densidad → **bombas** → huecos de perk. `STARTING_ORE_AMOUNT` es % de mineral suelto. Extras al destruir se aplican al *siguiente* lote revelado (tope 20%), nunca rellenan el túnel.
