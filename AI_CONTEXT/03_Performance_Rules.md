...
* El chunk **no** decide WHEN: solo acota WHERE. `OreSpawner` mantiene ~`starting_ore_amount` ores cerca del player (batch + refill + bias de direccion), con extras al destruir (`spawn_on_destroy_*`, clusters). Overflow solo en anillo adyacente al chunk.
* `OreSpawner` separa elegir celdas de instanciar: encola y respeta `max_spawns_per_frame`. Refill corre en tick (`refill_interval`) + `on_window_moved`.
