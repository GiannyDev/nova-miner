...
* El chunk **no** decide el contenido: solo acota WHERE. `OreSpawner` genera cada celda **una vez** al revelar (`UNKNOWN` → dirt / ore / hueco). Pipeline: spawn holes → rutas caminables → clusters → extras capados → densidad. `STARTING_ORE_AMOUNT` es % de mineral suelto. Extras al destruir (`SPAWN_ON_DESTROY_*`, `SPAWN_CLUSTER_*`) se acumulan y se aplican al *siguiente* lote revelado (tope 20% del lote), nunca rellenan el túnel ni pintan el anillo de Unknown.
