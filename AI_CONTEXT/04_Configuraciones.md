...
* MineSpawnProfile → `data/mines/` (safe zone, `dirt_data`/`dirt_hp`, `bomb_data`, clusters, walkable paths, `ore_weights`, **world perks** `perk_count` / `perk_min_distance_cells` / `perk_pool`). Densidad de mineral suelto = `Stats.STARTING_ORE_AMOUNT` (%). Tierra: `OreData.is_dirt`. Bomba: `OreData.is_bomb`. Helpers: `HelperData` (look + wander + `hit_delay`) + Stats `HELPERS_UNLOCKED` / `HELPER_SPEED` / `HELPER_DMG`. Player y helpers comparten `MinerBody` + `drill_base.tscn`. Offscreen markers: contrato `get_marker_world_pos` / `get_marker_icon` / `is_marker_active`.
