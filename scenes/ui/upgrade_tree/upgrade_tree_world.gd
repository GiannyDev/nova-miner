...
func _draw():
	for node in $Nodes.get_children():
		var upgrade_node := node as UpgradeNode
		if upgrade_node == null or not upgrade_node.visible:
			continue
		for connected in upgrade_node.get_connected_nodes():
			if connected.visible:
				draw_edge(upgrade_node, connected)
